...
export default function SystemAdmin() {
  ...
  const { fetchDashboard } = useServerStore()
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      console.error('[system-admin] save failed:', err)
    } finally {
  ...
  const handleAISave = async () => {
    ...
    try {
      ...
      console.error('[system-admin] save AI:', err)
    } finally {
      setAiSaving(false)
    }
  }
  ...
  const handleScheduleToggle = async (schedule: AISchedule) => {
    try {
      ...
      console.error('[system-admin] toggle schedule:', err)
    }
  }

  const fetchManaged = useCallback(() => {
    getManagedServers().then(setManagedServers).catch((err) => console.error('[system-admin] fetch managed:', err))
  }, [])

  const fetchCloud = useCallback(() => {
    getCloudAccounts().then(setCloudAccounts).catch((err) => console.error('[system-admin] fetch cloud:', err))
  }, [])

  const fetchNas = useCallback(() => {
    getNasDevices().then(setNasDevices).catch((err) => console.error('[system-admin] fetch nas:', err))
  }, [])

  const fetchCredentials = useCallback(() => {
    getCredentials().then(setCredentials).catch((err) => console.error('[system-admin] fetch credentials:', err))
  }, [])
  ...
  const handleNasSave = async () => {
    ...
    try {
      ...
      console.error('[system-admin] nas save:', err)
    } finally {
      setNasSaving(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Header */}
      <div className="mb-1">
        <h1 className="text-[22px] font-semibold text-[#495057]">系统设置</h1>
        <p className="text-sm text-[#878a99] mt-1">System Administration</p>
      </div>
