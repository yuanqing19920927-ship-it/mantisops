...
- **operator 创建 probe 的自动归属**：operator 创建探测规则（POST /probes）后，后端自动为该用户添加 `user_permissions` 记录（`res_type=probe, res_id=新规则ID`），确保创建者能立即看到和管理自己创建的探测规则。删除 probe 时自动清理所有用户的该 probe 权限记录。admin 创建资源无需此逻辑
- **告警规则不需要独立权限记录**：告警规则的可见性通过其 `target_id` 映射到 server/probe 维度自动过滤——规则的 target 在用户权限范围内则可见。operator 创建告警规则时，后端校验 `target_id` 必须在该用户的可见范围内（不能为空——全局规则仅 admin 可创建），创建成功后规则自然可见，无需额外权限记录
...
| 告警确认（ack） | operator | 仅可确认自己有权访问的目标的告警 |
| 探测规则 / 资产 / 通知渠道 CRUD | operator | |
| 告警规则 CRUD | operator | operator 只能 CRUD `target_id` 在其权限范围内的规则；全局规则（target_id 为空）仅 admin 可 CRUD |
| 用户管理 CRUD | admin | |
...
- `GET /alerts/events`、`GET /alerts/stats` — **按告警目标资源过滤**（见下方告警目标映射规则）
- `GET /alerts/events/:id/notifications` — 先查事件的 target_id，检查用户是否有权访问该目标，无权返回 403
- `GET /alerts/rules` — 按规则的 `target_id` 过滤。**全局规则**（`target_id` 为空，如 `server_offline` 类型不指定特定服务器）：所有用户可见（只读），**但仅 admin 可 CRUD 全局规则**。operator 只能 CRUD `target_id` 在其权限范围内的规则
- `GET /billing` — 按可见数据库/服务器过滤：通过 `cloud_instances` 表的 `instance_id → host_id` 映射，ECS 按 `PermissionSet.Servers` 过滤，RDS 按 `PermissionSet.Databases` 过滤，**SSL 证书仅 admin 可见**（包含域名等敏感信息，不关联特定资源）

**告警目标 target_id 映射规则**：

告警事件的 `target_id` 格式因规则类型而异，过滤时需要提取其中的 host_id 或 probe ID：

| 规则类型 | target_id 格式 | 过滤方式 |
|---------|---------------|---------|
| server_offline / cpu / memory / network_rx / network_tx / gpu_* | `{host_id}` | 直接检查 `Servers[target_id]` |
| disk | `{host_id}:{mount_point}` | 提取冒号前的 host_id，检查 `Servers[host_id]` |
| container | `{host_id}:{container_name}` | 提取冒号前的 host_id，检查 `Servers[host_id]` |
| probe_down | `{probe_rule_id}` | 直接检查 `Probes[target_id]` |

实现为 `PermissionSet.CanSeeAlertTarget(ruleType, targetID string) bool` 方法，内部根据 ruleType 选择解析策略。对于 disk/container 类型，使用 `strings.SplitN(targetID, ":", 2)[0]` 提取 host_id。
...
**告警广播过滤规则**：告警事件携带 `target_id`，格式因规则类型而异（见 §3.5 告警目标映射规则表）。广播时按以下规则过滤：
- `server_offline / cpu / memory / network_rx / network_tx / gpu_*` → target_id 是纯 host_id，检查 `PermissionSet.Servers[target_id]`
- `disk` → target_id 格式为 `{host_id}:{mount_point}`，提取冒号前的 host_id，检查 `PermissionSet.Servers[host_id]`
- `container` → target_id 格式为 `{host_id}:{container_name}`，同上提取 host_id 检查
- `probe_down` → target_id 是 probe rule ID 字符串，检查 `PermissionSet.Probes[target_id]`
