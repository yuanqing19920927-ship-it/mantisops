...
CREATE TABLE ai_reports (
    ...
    summary TEXT NOT NULL DEFAULT '',       -- 报告摘要（前 200 字），用于列表页和仪表盘卡片
    ...
    token_usage INTEGER DEFAULT 0,          -- LLM token 消耗量
    generation_time_ms INTEGER DEFAULT 0,   -- 生成耗时（毫秒）
...
CREATE TABLE ai_messages (
    ...
    token_usage INTEGER DEFAULT 0,         -- 本条消息的 token 消耗
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
...
手动触发 / 定时触发
  ...
  → 提取前 200 字作为 summary
  → 更新 ai_reports（status=completed, content=markdown）
...
使用标准 5 段 cron 表达式（分 时 日 月 周），Go 侧用 `github.com/robfig/cron/v3` 解析 cron 表达式计算下次执行时间。不引入 robfig/cron 的调度器本身，只用其解析能力，调度逻辑自己实现（与 MantisOps 现有的 ticker 模式一致）。
...
用户发送消息（通过 REST API）
  ...
  → 存储 user message 到 ai_messages
  → 注入系统上下文（见 7.2）
  → 组装消息列表（system + 历史消息 + 新消息）
  → Provider.Stream(messages) → token 流
  → 通过 WebSocket 逐 chunk 推送给客户端
  → 流结束后，完整 assistant 消息存入 ai_messages
  → 更新 ai_conversations.last_message_at
```
...
**上下文更新策略**：每次新对话的首条消息注入完整快照。同一对话的后续消息不重新注入（避免 token 浪费），除非用户显式要求"刷新数据"。

### 7.3 对话上下文窗口管理

- **最大历史消息数**：保留最近 20 条消息（10 轮对话）
- **超出时**：保留 system prompt + 最早 2 条 + 最近 16 条（保持对话连贯性）
- **长消息截断**：单条消息超过 4000 字符时，在发送给 LLM 前截断并追加 `[...内容已截断]`
...
```json
// AI 对话流式输出（逐 token 推送）
{"type": "ai_chat_chunk", "conversation_id": 5, "message_id": 43, "content": "根据", "done": false}
{"type": "ai_chat_chunk", "conversation_id": 5, "message_id": 43, "content": "本周数据", "done": false}
{"type": "ai_chat_chunk", "conversation_id": 5, "message_id": 43, "content": "", "done": true, "token_usage": 350}
...
**广播策略**：`ai_chat_chunk` 仅推送给发起对话请求的客户端连接（需在 Hub 中标记请求来源的 client）。`ai_report_completed` / `ai_report_failed` 广播给所有连接。

### 9.1 Hub 改造

当前 Hub 只有 `BroadcastJSON()`（全量广播）和 `BroadcastLogJSON()`（仅日志订阅者）。AI 对话的流式输出需要**定向推送**，新增：
...
客户端在发起对话消息的 HTTP 请求时，携带 `X-WS-Client-ID` 头（前端在 WebSocket 连接建立时生成唯一 ID），后端据此找到对应的 WebSocket 连接进行定向推送。
...
**API Key 存储**：Claude/OpenAI 的 API Key 属于敏感信息。首次在设置页配置时，通过现有的 AES-256-GCM `crypto` 模块加密后存入 `settings` 表（key 为 `ai.claude.api_key` 等），不明文写入 server.yaml。server.yaml 中的 api_key 字段仅作为初始导入用途（与阿里云 AK/SK 的处理方式一致）。
