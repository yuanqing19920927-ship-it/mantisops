...
CREATE UNIQUE INDEX idx_ai_reports_type_period_unique ON ai_reports(report_type, period_start) WHERE status != 'failed';
```
...
- `force` 可选，默认 false。当已存在同类型同时间窗口的 completed 报告时：不带 force 返回 `409 Conflict`（附带已存在报告 ID），`force=true` 则删除旧报告后重新生成。
...
**广播策略**：`ai_chat_chunk` 仅推送给订阅了对应 `stream_id` 的客户端连接。`ai_report_completed` / `ai_report_failed` 广播给所有连接。
...
**流程：**
1. 前端通过 REST API 发送对话消息，后端返回 `stream_id`（UUID）
2. 前端通过 WebSocket 发送订阅消息：`{"type": "ai_stream_subscribe", "stream_id": "xxx"}`
3. 后端在 Hub 中记录该 client 订阅的 stream_id
