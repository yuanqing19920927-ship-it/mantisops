...
CREATE UNIQUE INDEX idx_ai_reports_type_period_unique ON ai_reports(report_type, period_start) WHERE status = 'completed';
```
...
手动触发 / 定时触发
  ...
  → 如果是 force 覆盖：将同类型同窗口的旧 completed 报告标记为 superseded
...
用户发送消息（通过 REST API）
  ...
  → 预创建空的 assistant message 到 ai_messages（message_id=43）
  ...
  → 流结束后，回填 assistant message 的 content + token 统计
...
- `force` 可选，默认 false。当已存在同类型同时间窗口的 completed 报告时：不带 force 返回 `409 Conflict`（附带已存在报告 ID），`force=true` 则启动新报告生成，**新报告成功后再将旧报告标记为 `superseded`**（不立即删除，保留历史记录）。如果新报告生成失败，旧报告保持不变。
...
| WebSocket 消息 | 新增 ai_chat_chunk / ai_report_generating / ai_report_completed / ai_report_failed 类型 |
